    ...
    for p in tokens:
        ...
        if '-' in hostpart:
            ...
            if sep and tail and re.match(r'^[A-Za-z0-9_\-]+$', tail) and head.strip():
                ...
                hostpart = hostpart
                lbl = ""
        # Now handle hostpart: try to extract trailing :port using rsplit
        if ':' in hostpart:
            # Попытка: разделить по последнему ':' и проверить, что правая часть — цифры (порт)
            ...
            if prt.isdigit():
                ...
                # Не digits: это либо hostname-with-colon (маловероятно) либо IPv6 без port.
                # В этом случае считаем hostpart целиком хостом и используем default_port.
                out.append({"host": hostpart.strip(), "port": default_port, "label": lbl})
                continue
        else:
            # Просто хост без порта
            h = hostpart.strip()
