...
# Новая папка для произвольных файлов, которые при наличии будут добавлены в каждый ZIP-архив.
# Папка опциональна — может быть пустой и не использоваться.
# Все содержимое (файлы и подпапки) этой папки будет добавлено в корень ZIP-архива,
# сохраняя относительную структуру внутри папки file.
FILE_DIR = SCRIPT_DIR.joinpath("file")
...
def atomic_write_text(path: pathlib.Path, text: str, encoding: str = "utf-8") -> None:
    ...
    try:
        # Открываем fd напрямую, чтобы избежать промежуточных проблем с правами
        with os.fdopen(fd, "w", encoding=encoding, newline="\n") as f:
            f.write(text)
        os.replace(tmpname, str(path))
    finally:
        # В редком случае временный файл может остаться — пытаемся удалить
        try:
...
def exec_cmd(cmd, input: Optional[str] = None, shell: bool = False, timeout: Optional[int] = None) -> Tuple[int, str]:
    ...
    Небольшая аккуратность: если cmd -- строка, по умолчанию используется shell=True,
    если cmd -- список, используется shell=False. Это историческое поведение сохранено.
    """
    try:
        # Поддержим оба варианта: строка -> shell=True (если явно не переопределено),
        # список -> shell=False
        use_shell = shell
...
def gen_preshared_key() -> str:
    ...
    # Если openssl не доступен — используем fallback на Python
    rc, out = exec_cmd(["openssl", "rand", "-base64", "32"])
    if rc == 0 and out:
        return out.strip()
    # Fallback: используем os.urandom
    try:
        import base64 as _b64, os as _os
        return _b64.b64encode(_os.urandom(32)).decode("ascii")
    except Exception:
...
AllowedIPs = <SERVER_ADDR>, <ALLOWED_IPS>
"""
...
def zip_client_files(client_name: str) -> None:
    ...
    Дополнительно: весь контент папки FILE_DIR (файлы и подпапки) будет добавлен в корень архива,
    сохраняя относительную структуру относительно FILE_DIR. Лог о добавлении файлов из file
    удалён по пожеланию — функция работает тихо, только логирует предупреждения/ошибки.
    """
    ...
    # Создаём ZIP (перезаписываем если уже существует)
    with zipfile.ZipFile(str(zip_filename), 'w', compression=zipfile.ZIP_DEFLATED) as zipf:
        # Добавляем конфиги и png из CONF_DIR
        ...
        # Рекурсивно добавляем содержимое FILE_DIR (файлы и каталоги) в корень архива,
        # сохраняя относительную структуру относительно FILE_DIR.
        if FILE_DIR.exists() and FILE_DIR.is_dir():
            for root, dirs, files in os.walk(FILE_DIR):
                # Добавляем пустые директории (если нужно)
                rel_root = os.path.relpath(root, FILE_DIR)
                if rel_root == ".":
                    rel_root = ""
                else:
                    # создаём запись директории в ZIP (чтобы пустые директории тоже сохранились),
                    # имя директории в ZIP должно оканчиваться '/'
                    dir_arcname = rel_root.rstrip("/") + "/"
                    try:
                        zinfo = zipfile.ZipInfo(dir_arcname)
                        # установить флаги и время, но это необязательно
                        zipf.writestr(zinfo, "")
                    except Exception:
                        # Игнорируем ошибки при добавлении директории
                        pass
                for fname in files:
                    full_path = os.path.join(root, fname)
                    # arcname — относительный путь внутри архива, без префикса папки file
                    if rel_root:
...
# Доп. опция: показать/использовать папку file (необязательная)
parser.add_argument("--file-dir", default=str(FILE_DIR), help="Каталог с дополнительными файлами, которые будут добавлены в ZIP (по умолчанию ./file)")
...
def main() -> None:
    # Обновляем FILE_DIR в соответствии с опцией, если пользователь её передал
    global FILE_DIR
    if opt.file_dir:
        try:
            FILE_DIR = pathlib.Path(opt.file_dir)
            FILE_DIR.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            logger.warning("Не удалось создать/использовать file-dir '%s': %s", opt.file_dir, e)
