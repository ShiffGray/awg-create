...
find_tunnel_by_subnet_in_configs() {
  ...
  for conf_file in "$script_dir"/*.conf 2>/dev/null; do
...
for rule in "${LAN_ALLOW[@]}"; do
  ...
  for part in "${PARTS[@]}"; do
    ...
    # 1. Сначала ищем в .conf файлах (надёжнее)
    tun_name=$(find_tunnel_by_subnet_in_configs "$part")
    
    # 2. Если не нашли — ищем по активным интерфейсам
    if [ -z "$tun_name" ]; then
...
# Удаляем файл параметров туннеля (он больше не нужен)
rm -f "$STATE_BASE_DIR/.tunnels/${TUN}.params"
...
# --- Читаем все параметры из временного файла .params ---
# Файл хранится в .state/.tunnels/${TUN}.params
TUNNELS_STATE_DIR="$STATE_BASE_DIR/.tunnels"
TUNNEL_PARAMS_FILE="$TUNNELS_STATE_DIR/${TUN}.params"
...
# Читаем INTERFACE_MAP из .params файла (вместе со всеми параметрами)
# LAN_ALLOW_FILE больше не используется — всё в .params
# INTERFACE_MAP уже готов — up.sh всё записал! Не нужно искать!
